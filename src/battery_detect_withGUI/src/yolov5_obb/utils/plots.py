...
from utils.general import (LOGGER, Timeout, check_requirements, clip_coords, increment_path, is_ascii, is_chinese,clip_polys,
...
def save_one_box(poly, im, file='image.jpg', gain=1.02, pad=10, square=False, BGR=False, save=False):
    # Save image crop as {file} with crop size multiple {gain} and {pad} pixels. Save and/or return crop
    poly = torch.tensor(poly).view(-1,8)
    #clip_polys(poly, im.shape)
    poly = torch.tensor(poly).view(1,4,2)
    rect = cv2.minAreaRect(poly.numpy())
    ...
    output_size = (256, 256)  # Desired output size
    #result_image = insert_into_white_background(warped, output_size)
    #x1, y1, x2, y2, x3, y3, x4, y4 = poly[0].tolist()
    #crop = im[int(y1):int(y3), int(x1):int(x2), ::(1 if BGR else -1)]
    if save:
        file.parent.mkdir(parents=True, exist_ok=True)  # make directory
        cv2.imwrite(str(increment_path(file).with_suffix('.jpg')), warped)
    return warped

def insert_into_white_background(cropped_image, output_size):
    # Create a white background image of desired size
    white_background = np.ones((output_size[1], output_size[0], 3), dtype=np.uint8) * 255  # Create white image

    # Calculate position to place the cropped image at the center of the white background
    x_offset = (output_size[0] - cropped_image.shape[1]) // 2
    y_offset = (output_size[1] - cropped_image.shape[0]) // 2

    # Insert the cropped image into the white background
    white_background[y_offset:y_offset + cropped_image.shape[0], x_offset:x_offset + cropped_image.shape[1]] = cropped_image
