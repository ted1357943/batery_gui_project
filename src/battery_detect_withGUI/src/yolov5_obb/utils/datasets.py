...
class LoadImages_HSV:
    ...
    def __next__(self):
        ...
        # Padded resize
        img = letterbox(merge_image, self.img_size, stride=self.stride, auto=self.auto)[0]
...
class LoadStreams:
    ...
    def __init__(self, sources='streams.txt', img_size=(), stride=32, auto=True):
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
...
class LoadStreams_HSV:
    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True):
        ...
        n = len(sources)
        self.imgs, self.fps, self.frames, self.threads, self.img0 = [None] * n, [0] * n, [0] * n, [None] * n, [None] * n
        self.sources = [clean_str(x) for x in sources]  # clean source names for later
        self.auto = auto
        for i, s in enumerate(sources):  # index, source
            # Start thread to read frames from video stream
            ...
            _, self.imgs[i] = cap.read()  # guarantee first frame
            ###第一幀改成HSV###
            self.img0[i] = self.imgs[i].copy()
            self.imgs[i] = cv2.cvtColor(self.imgs[i], cv2.COLOR_BGR2HSV) # Convert BGR to HSV
            hue, saturation, value = cv2.split(self.imgs[i])
            zero_channel = np.zeros_like(hue)*100
            self.imgs[i] = cv2.merge([hue, zero_channel, value])
            ###  end  ###
            self.threads[i] = Thread(target=self.update, args=([i, cap, s]), daemon=True)
            LOGGER.info(f"{st} Success ({self.frames[i]} frames {w}x{h} at {self.fps[i]:.2f} FPS)")
            self.threads[i].start()
    ...
    def update(self, i, cap, stream):
        ...
        while cap.isOpened() and n < f:
            ...
            if n % read == 0:
                ...
                zero_channel = np.zeros_like(hue)*100
                im = cv2.merge([hue, zero_channel, value])
                ###  end  ###
                if success:
                    self.img0[i] = im0
                    self.imgs[i] = im
    ...
    def __next__(self):
        ...
        #img = [letterbox(x, self.img_size, stride=self.stride, auto=self.rect and self.auto)[0] for x in img0]
        img = [letterbox(x, self.img_size, stride=self.stride, auto=self.rect and self.auto)[0] for x in self.imgs]
        # Stack
        img = np.stack(img, 0)
        ...
        return self.sources, img, self.img0, None, ''
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, cls_names, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if self.rect:
            ...
            self.batch_shapes = np.ceil(np.array(shapes) * img_size / stride + pad).astype(np.int64) * stride # (nb, [h_rect, w_rect])
