...
def non_max_suppression_obb(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            i, j = (x[:, 5:class_index] > 0).nonzero(as_tuple=False).T # ()
...
def apply_YOLO(x, model, im, im0):
    names = model.names
    pred = model(im, augment=False, visualize=False)
    pred = non_max_suppression_obb(pred, 0.25, 0.45, None, False, multi_label=True, max_det=1000)
