...
def run(data,
    ...
    ###存成.csv和.npz###
    if plots:
        np.savez(Path(save_dir)/'confusion_matrix.npz', matrix=confusion_matrix.matrix)
        np.savetxt(Path(save_dir)/'confusion_matrix.csv', confusion_matrix.matrix, delimiter=',')
        ###normalize 可以選True or False
